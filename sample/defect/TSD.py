...
class TSDDataset(utils.Dataset): # bai

    def load_TSD(self, dataset_dir, subset): # bai
        """Load a subset of the Pothole dataset.
        dataset_dir: Root directory of the dataset.
        subset: Subset to load: train or val
        """
        # Add classes. We have only one class to add.
        self.add_class("TSD", 1, "TSD")  # bai
...
def train(model):
    """Train the model."""
    # Training dataset.
    dataset_train = TSDDataset() # bai
    dataset_train.load_TSD(args.dataset, "train") # bai
    dataset_train.prepare()

    # Validation dataset
    dataset_val = TSDDataset() # bai
    dataset_val.load_TSD(args.dataset, "val") # bai
    dataset_val.prepare()
...
def color_splash(image, mask):
    ...
    if mask.shape[-1] > 0:
        ...
        splash = np.where(mask, img_red, gray).astype(np.uint8)
    else:
...
def detect_and_show(model, image_path=None, video_path=None): # bai
    ...
    if image_path:
        # Validation dataset
        dataset = TSDDataset()  # bai
        dataset.load_TSD(args.dataset, "val")  # bai
        dataset.prepare()
        # Run model detection and generate the color splash effect
        print("Running on {}".format(args.image))
        # Read image
        ...
        visualize_cv2.save_image(image, file_name, r['rois'], r['masks'],
                             ...
                             filter_classs_names=['TSD'], scores_thresh=0.7, mode=0)
    elif video_path:
        import cv2
        # Video capture
        vcapture = cv2.VideoCapture(video_path)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Train Mask R-CNN to detect TSDs.')
    parser.add_argument("command",
                        metavar="<command>",
                        help="'train' or 'splash'")
    parser.add_argument('--dataset', required=False,
                        metavar="/path/to/balloon/dataset/",
                        help='Directory of the TSD dataset')
    ...
    # Configurations
    if args.command == "train":
        config = TSDConfig() # bai
    else:
        class InferenceConfig(TSDConfig): # bai
